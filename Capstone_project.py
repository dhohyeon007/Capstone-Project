...
# 재시도 옵션 설정 (지수 백오프)
gemini_exp_backoff = types.HttpRetryOptions(
    attempts=8,
    initial_delay=12.0,
    max_delay=90.0,
    exp_base=1.5,
...
client = genai.Client(
    ...
    http_options=types.HttpOptions(retry_options=gemini_exp_backoff)
)
...
def upload_file():
    ...
    while True:
        ...
        items = [item for item in os.listdir(current_dir) if item.endswith(".pdf") or item.endswith(".md") or os.path.isdir(os.path.join(current_dir, item))]
        ...
        if choice == "-1":
            ...
            try:
                ...
                #     if selected_item.lower().endswith(".pdf"):
...
# 각 청크를 처리하는 함수
def process_chunk(filepath, start_page_num):
    ...
    try:
        ...
        print(f"Error processing chunk {filepath}: {e}")
...
def ocr_model():
    # 임시 파일 생성을 위해 업로드된 파일을 안전한 위치로 복사
    file_name = upload_file()
    os.makedirs("temp_ocr", exist_ok=True)
    safe_file_name = os.path.join(os.getcwd(), "temp_ocr", "temp_upload_file.pdf")
    ...
    try:
        ...
        # 각 청크를 병렬로 처리하여 OCR 결과를 마크다운 파일로 저장
